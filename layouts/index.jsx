	...
	if (!mounted || status === 'loading') {
		return (
			<Flex justify="center" align="center" h="100vh" w="full">
				...
				<Spinner position="absolute" boxSize={16} thickness={2} speed="0.8s" emptyColor="canvas-1" color="brand.default" />
			</Flex>
		)
	} else {
		...
		if (session && session.user.status === 'Inactive') {
			return (
				<>
					...
					<Container>
						<Flex bg="red.alpha" justify="center" gap={3} border="1px solid" borderColor="red.default" borderRadius={12} p={6} color="red.default">
							<Icon as={FiAlertTriangle} boxSize={6} />
							<Text fontWeight="medium">Your account is inactive.</Text>
						</Flex>
					</Container>
				</>
			)
		...
		return (
			<>
				...
				<chakra.div mx="auto" h="auto" minH="calc(100vh - 72px)" w="full" maxW={isAdmin ? 1536 : 1280}>
					...
					<chakra.main ml={{ base: 0, lg: isAdmin ? 256 : isUserPage ? 256 : 0 }} w={{ base: 'full', lg: isAdmin ? 'calc(100% - 256px)' : isUserPage ? 'calc(100% - 256px)' : 'full' }}>
						{props.children}
					</chakra.main>
				</chakra.div>
