...
import { Badge, Button, chakra, Container, Flex, IconButton, Image, Select, Td, Text, Tr } from '@chakra-ui/react'
					...
					<Table
							...
							<Tr key={product._id}>
								<Td maxW={200}>
									<Flex align="center" gap={3}>
										<Image borderRadius={8} boxSize={8} alt={product.name} src={product.image} />
								...
								<Td>
									<Badge variant="tinted" colorScheme={product.status === 'Published' ? 'blue' : product.status === 'Draft' ? 'yellow' : product.status === 'Disabled' && 'red'} textTransform="capitalize">
										{product.status}
									</Badge>
								</Td>

								<Td>
									<Text>{month[product.created.split(',')[0].split('/')[0] - 1] + ' ' + product.created.split(',')[0].split('/')[1] + ', ' + product.created.split(',')[0].split('/')[2]}</Text>
								</Td>

								<Td textAlign="right">
									<IconButton size="xs" icon={<FiMoreHorizontal size={12} />} onClick={() => router.push(`/admin/products/${product._id}`)} />
								</Td>
							</Tr>
						)}
						select={(register) => (
							<Flex flex={1} justify="end" align="center" gap={3}>
								<Select placeholder="Status" size="lg" w={{ base: 'full', md: 'auto' }} {...register('status')}>
									<chakra.option value="Published">Published</chakra.option>
						...
						filters={(data, watch) => {
							return data
								...
								.filter((data) => (watch('status') ? data.status === watch('status') : data))
						}}
